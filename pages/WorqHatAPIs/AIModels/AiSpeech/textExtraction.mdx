...
We're introducing SpeechAI, a state-of-the-art speech recognition model trained on 850K hours of
audio data that achieves near human-level performance and robustness across a variety of data.

## How does it work?

The SpeechAI is a neural net for speech recognition that is built over a Public Research that was published by Google Brain in 2020. The SpeechAI builds upon the now-ubiquitous Transformer architecture, which is famous for its parallelizability and heavy use of the attention mechanism. By integrating convolutional layers into the Transformer architecture, the model can capture both local and global dependencies while being a relatively size-efficient neural net architecture thus enabling users to run it directly in their clients.
